...
def ring_PhC(
    ...
    c1 = gf.Component()
    c2 = gf.Component()
    width = loadmat(width_file)["width_design"].flatten()
    points = len(width)
    theta = np.linspace(0, 2*np.pi, points)
    vertices_in = [(radius+width_bend/2-width)*np.cos(theta), 
                   (radius+width_bend/2-width)*np.sin(theta)]
    circle_inside = c1.add_polygon(np.array(vertices_in).T, layer=Sections.LAYER.NR)
    
    theta_downsampled = np.linspace(0, 2*np.pi, points//5)
    vertices_out = [(radius+width_bend/2)*np.cos(theta_downsampled), (radius+width_bend/2)*np.sin(theta_downsampled)] 
...
if __name__ == "__main__":
    c = ring_PhC(width_file="C:\\Lumerical Simulation\\TM_PhC_Ring\\theta_width_1060_220_TM_SiN_A=0.002.mat")
    c.plot()
    c.show()
